...
for i, html_file in enumerate(tqdm(html_files, desc="Processing race results")):
    ...
    for j, row in enumerate(rows):
        ...
        jockey = ''.join(re.findall(r'[\u4e00-\u9fff\u3040-\u309f\u30a0-\u30ffA-Za-z]+', jockey))
        # 単勝オッズ
        odds_tag = row.select_one('td.Odds span.Odds_Ninki')

        if odds_tag is None:
            # 2着馬などclassがついていない場合もある
            odds_tag = row.select_one('td.Odds span')
        odds = odds_tag.text.strip()

        if jockey in target_jokey_name:
            # レース情報がまだ登録されていない場合は初期化
            if race_info not in target_results_dict[jockey].keys():
                target_results_dict[jockey][race_info] = {}
                target_results_dict[jockey][race_info]["race_results"] = deepcopy(target_race_results)
                target_results_dict[jockey][race_info]["return_rate"] = deepcopy(target_return_rate)

            # 1着の場合
            if j == 0:
                target_results_dict[jockey][race_info]["race_results"]['1着'] += 1
                target_results_dict[jockey][race_info]["return_rate"]['単勝回収率'] += float(odds)
                target_results_dict[jockey][race_info]["return_rate"]['複勝回収率'] += float(payouts[0])/100
            # 2着の場合
            if j == 1:
                target_results_dict[jockey][race_info]["race_results"]['2着'] += 1
                target_results_dict[jockey][race_info]["return_rate"]['複勝回収率'] += float(payouts[1])/100
            # 3着の場合
            if j == 2:
                if len(payouts) < 3:
                    ...
                    target_results_dict[jockey][race_info]["return_rate"]['複勝回収率'] += float(payouts[2])/100
            # 4着以下の場合
            if j >= 3:
                target_results_dict[jockey][race_info]["race_results"]['4着以下'] += 1
...
for jockey in target_results_dict.keys():
    for key in target_results_dict[jockey].keys():
        target_race_results_tmp = target_results_dict[jockey][key]["race_results"]
        target_return_rate_tmp = target_results_dict[jockey][key]["return_rate"]

        race_count = sum([target_race_results_tmp[key] for key in target_race_results_tmp.keys()])
        target_return_rate_tmp['単勝回収率'] = (target_return_rate_tmp['単勝回収率']) / race_count
        target_return_rate_tmp['複勝回収率'] = (target_return_rate_tmp['複勝回収率']) / race_count
